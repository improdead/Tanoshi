...
model_cache = modal.Volume.persisted("ai-manga-model-cache")
...
# Persistent dicts for job tracking
analysis_jobs = modal.Dict.from_name("ai_analysis_jobs", create_if_missing=True)
audio_jobs = modal.Dict.from_name("ai_audio_jobs", create_if_missing=True)


# Image/runtime setup (FastAPI for web; GPU for workers)
# Includes audio tooling and model deps.
image = (
    modal.Image.debian_slim()
    .apt_install("ffmpeg", "libsndfile1")
    .pip_install(
        [
            "fastapi==0.115.0",
            "uvicorn[standard]==0.30.6",
            # Core ML + audio stack (installed alongside GPU torch below)
            "transformers",
            "Pillow",
            "TTS",
            "librosa",
...
def _synthesize_silence_wav_base64(duration_seconds: float = 0.25, sample_rate: int = 22050) -> str:
...
def _make_placeholder_result(num_pages: int) -> Dict[str, Any]:
        ...
        pages.append(
            {
                ...
                "textRegions": [
                    {
                        "id": 0,
                        "text": f"Auto-generated dialogue for page {i}",
                        "boundingBox": {"x": 24.0, "y": 24.0, "width": 320.0, "height": 64.0},
                        "confidence": 0.95,
                        "isEssential": True,
                    }
                ],
                "characterDetections": [],
                # Mapping must be string keys in JSON
...
# -------------------------
# MAGI + XTTS pipeline (GPU-backed persistent container)
# -------------------------


@app.cls(image=image, gpu=modal.gpu.A10G(), timeout=60 * 30, concurrency_limit=1)
class AIMangaPipeline:
    def __enter__(self):
        # Lazy-import heavy libs inside the container
        ...
        # Device selection happens inside libs; Modal provides the GPU
        self._magi = AutoModel.from_pretrained(
            "ragavsachdeva/magiv2", trust_remote_code=True
        ).eval()

        # Prefer XTTS‑v2; fall back to a simpler model
        try:
            self._tts = TTS("tts_models/multilingual/multi-dataset/xtts_v2", gpu=True)
        except Exception:
            self._tts = TTS("tts_models/en/ljspeech/tacotron2-DDC", gpu=True)
    ...
    def run_analysis(self, job_id: str, pages_b64: List[str], character_bank: Dict[str, Any]) -> None:
        ...
        # Decode pages
        chapter_pages = [
            img for img in (self._read_image_from_base64(b) for b in pages_b64) if img is not None
        ]
        if not chapter_pages:
            analysis_jobs[job_id] = {"status": "failed", "error": "no valid images"}
            return

        # Decode character bank reference images
        character_images = []
        for img_b64 in (character_bank or {}).get("images", []):
            img = self._read_image_from_base64(img_b64)
            if img is not None:
        ...
        # Build result in the exact shape the iOS client expects
        pages_json: List[Dict[str, Any]] = []
        transcript: List[Dict[str, Any]] = []

        for i, page_result in enumerate(per_page):
            # speaker name mapping from MAGI associations
            speaker_name = {t: page_result["character_names"][c] for t, c in page_result["text_character_associations"]}
            ...
            # Convert associations from list of pairs -> dict of string keys
            assoc_dict = {str(t): int(c) for t, c in page_result.get("text_character_associations", [])}
...
@api.post("/analyze")
def start_analyze(body: Dict[str, Any]) -> Dict[str, str]:
    # Body schema: { pages: [base64...], characterBank: { images: [], names: [] } }
    pages: List[str] = body.get("pages", [])
    job_id = str(uuid.uuid4())
    analysis_jobs[job_id] = {"status": "pending", "progress": 0.0}
...
def get_status(job_id: str) -> Dict[str, Any]:
    state = analysis_jobs.get(job_id)
    if not state:
        # Match client expectation where 404 -> jobNotFound
        raise HTTPException(status_code=404, detail="job not found")

    # Response keys use snake_case to align with Swift's convertFromSnakeCase
    return {
        "job_id": job_id,
        "status": state.get("status", "pending"),
        "progress": float(state.get("progress", 0.0)),
...
def get_result(job_id: str) -> Dict[str, Any]:
    ...
    # Directly return the result object shape expected by the client
    return state["result"]


@api.post("/audio")
def start_audio(body: Dict[str, Any]) -> Dict[str, str]:
    # Body schema: { transcript: [ { pageIndex, textId, speaker, text, emotion? } ], voiceSettings: {...} }
    transcript: List[Dict[str, Any]] = body.get("transcript", [])

    job_id = str(uuid.uuid4())
    audio_jobs[job_id] = {"status": "processing"}

    # Generate synchronously so the first client poll to /audio/result succeeds
    AIMangaPipeline().run_audio.call(job_id, transcript, body.get("voiceSettings", {}))
    return {"job_id": job_id}
